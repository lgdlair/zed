...
impl Panel for AgentPanel {
    ...
    fn icon(&self, _window: &Window, cx: &App) -> Option<IconName> {
        (self.enabled(cx) && AgentSettings::get_global(cx).button).then_some(IconName::NeoAssistant)
    }
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
            ...
            Rc::new(move |window, cx| {
                ...
                Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                    menu.context(focus_handle.clone())
                        ...
                        .item(
                            ContextMenuEntry::new("Neo Agent")
        ...
        let toolbar_content = if use_v2_empty_toolbar {
            ...
            let agent_icon = if let Some(icon_path) = selected_agent_custom_icon_for_button {
                ...
                let icon_name = selected_agent_builtin_icon.unwrap_or(IconName::NeoAgent);
