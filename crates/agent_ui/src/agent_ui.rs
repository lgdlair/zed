...
impl Agent {
    ...
    pub fn label(&self) -> SharedString {
        match self {
            Self::NativeAgent => "Neo Agent".into(),
...
fn update_command_palette_filter(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _| {
        ...
        if disable_ai {
            ...
            filter.hide_action_types(&[TypeId::of::<zed_actions::OpenNeoPredictOnboarding>()]);
        } else {
            if agent_enabled {
            ...
            filter.show_action_types(&[TypeId::of::<zed_actions::OpenNeoPredictOnboarding>()]);
