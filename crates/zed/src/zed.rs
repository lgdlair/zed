...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("NEO_ALLOW_EMULATED_GPU").is_err() {
        ...
        let message = format!(
            db::indoc! {r#"
            Neo uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set NEO_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
